...
class _LinkageViewState<T extends BaseItem> extends State<LinkageView> {
  ...
  Widget groupItem(BuildContext context, int index) {
    BaseItem item = groups[index];
    return GestureDetector(
      onTap: () {
        print("$index tap");
        if (mounted) {
          setState(() {});
          widget.onGroupItemTap!(context, index, groups[index]);
          selectIndex = index;
          double tempLength = 0.0;
          for (var i = 0; i < groups[index].index!; i++) {
  ...
  resetGroupPosition() {
    if (containerSize != null) {
      ...
      if (currentLength > containerSize!.height / 2 &&
          groups.length * widget.itemGroupHeight > containerSize!.height) {
        offset = ((currentLength - containerSize!.height / 2) / widget.itemGroupHeight.round()) * widget.itemGroupHeight;
        if (offset + containerSize!.height > groups.length * widget.itemGroupHeight) {
          offset = groups.length * widget.itemGroupHeight - containerSize!.height;
        }
